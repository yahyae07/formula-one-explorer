...
const HeaderBar = () => {
  const { showCardView, toggleCardView, showListView, toggleListView } =
    useViewStore();
  ...
  return (
    <div className="relative flex h-16 w-full flex-row items-center justify-between bg-[var(--f1-grey)] shadow-lg px-5">
      ...
      <div className="flex flex-row justify-center gap-2">
        <label className="relative block aspect-[2/0.75] w-20 rounded-full bg-gradient-to-br from-[var(--f1-lilac)] via-[var(--f1-red)] shadow-2xl transition-all duration-300 hover:bg-[length:100%_500%] focus:bg-[length:100%_500%] bg-[length:100%_100%] hover:cursor-pointer">
          <input
